...
static constexpr const port_id_t port_ecu_id{10};
static constexpr const port_id_t port_fuel_id{11};

const uint8_t MSG_ECU_SIZE{7};  //ECU
...
uint8_t m_snd_fuel_buf[MSG_FUEL_SIZE] = {};
...
const char *EcuStrError(uint32_t id)
{
    ...
    case 14: {
        return "starter_ctl_htmp...";
    }
    case 15: {
        return "pump_ctl_htmp...";
    }
    case 16: {
        return "clutch_error...";
    }
...
const char *EcuStrStatus(uint32_t id)
{
    ...
    case 6: {
        return "run_learn_stick_max...";
    }
    case 7: {
        return "run_learn_stick_min...";
    }
    case 8: {
        return "run_learn_rc...";
    }
    case 9: {
        return "run_stick_min...";
    }
    case 10: {
        return "run_pump_limit...";
    }
    case 11: {
        return "running...";
    }
    ...
    case 14: {
        return "test_glowplug...";
    }
    case 15: {
        return "test_valve_main...";
    }
    case 16: {
        return "test_valve_ignite...";
    }
    case 17: {
        return "test_oilpump...";
    }
...
void print_and_save_param()
{
    ...
    m_rpm::publish((float) m_engine_rpm / 10.f);
    m_egt::publish((float) m_engine_temp / 10.f);
...
int main()
{
    task("on_main_task", 100);
    task("on_ecu_task", 100);
    task("on_fuel_task", 500);

    receive(port_ecu_id, "on_ecu_serial");
...
EXPORT void on_ecu_task()
{
    ...
    if (len) {
        send(port_ecu_id, cmd, len, false);
    }
}
//---------------------------------------------------------------
...
{
    if (size != MSG_ECU_SIZE) { //MSG_ECU_SIZE
        return;
    }

    memcpy(&m_rev_buf, data, MSG_ECU_SIZE);

    if (calc_crc8(m_rev_buf, MSG_ECU_SIZE - 1) == m_rev_buf[MSG_ECU_SIZE - 1]) {
        state_comm();
...
{
    m_snd_fuel_buf[0] = 0x31;
    m_snd_fuel_buf[2] = 0x06;

    m_snd_fuel_buf[1] = ADR_FUEL[idx_fuel_array++];
    if (idx_fuel_array == 3) {
        idx_fuel_array = 0;
    }

    m_snd_fuel_buf[3] = calcCRC(m_snd_fuel_buf, 3);
    send(port_fuel_id, m_snd_fuel_buf, 4, true);
}
//---------------------------------------------------------------
EXPORT void on_fuel_serial(const uint8_t *data, size_t size)
{
    ...
    float fuel = (float) (data[4] | (data[5] << 8)) / 10.0f;

    if (data[1] == ADR_FUEL[0]) {
        m_fuel1::publish(fuel);
        //printf("f1: %.2f\r\n", fuel);
    } else if (data[1] == ADR_FUEL[1]) {
        m_fuel2::publish(fuel);
        //printf("f2: %.2f\r\n", fuel);
    } else if (data[1] == ADR_FUEL[2]) {
        m_fuel3::publish(fuel);
        //printf("f3: %.2f\r\n", fuel);
    }
}
