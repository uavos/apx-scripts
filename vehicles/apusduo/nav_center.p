...
#define SET_RPM 8
#define SET_HANDBRAKE 10
#define SET_DETECT 11

#define SET_SW_FREQUENCE 120

#define BATTERY_MAX 29
#define ALT_START_RPM 3500
#define ALT_STOP_RPM 3000
#define ALT_CURRENT_PHASE 7.0
...
// For VESC
#define PACKED_SIZE 78//70

// For BLDC
//#define PACKED_SIZE 63

/*****************************/
#define TANK_CAPACITY 14.1
#define TANK_OFFSET 0.18
/****************************/
///
...
//===========================
const LINK_AVAILABLE_TIMEOUT = 60;      //seconds
...
@OnTask()
{
    ...
    if(cntStarterRequest++ >= 2){
        ...
        if(tmp++==20) {
            ...
            new red_zone = get_var(f_VM20);
            if (get_var(f_userb_5) && red_zone >= 1.0 ) {
    ...
    if(!isLinkAvailable && (!get_var(f_power_xpdr) || get_var(f_mode)!=8))
    {
      set_var(f_mode, 8, true);
      set_var(f_power_xpdr, 1, true);
      set_var(f_cmode_throvr, 0, true);
      print("VM:Start safety proc...\n");
    }
    return 5;
}
...
//  printf("crc_error = %d\n", crc_error);
  ...
  switch(msg_type){                                         // inline func in PAWN?
    case GET_VALUES:{
        new param;
...
createTelemetryPack()
{
  ...
  if(pack_len > 13) pack2BytesSFloat(13, phase_current);        //Mandala
