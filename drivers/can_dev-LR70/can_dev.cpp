...
using fb_tail_rpm = Mandala<mandala::est::env::usrx::x1>;
...
bool processEcuPackage(uint32_t can_id, const uint8_t *data)
{
    switch (can_id) {
    case ECU_CTRL_ID: // TODO CHECK ENDIAN
        uint16_t engine_speed = data[0] | data[1] << 8;
        uint16_t engine_demand;
        uint16_t engine_tps;
        uint16_t engine_throttle;
    ...
    bool processTailPackage(uint32_t can_id, const uint8_t *data)
    {
        if ((can_id & 0xFF) == VESC_TAIL_CAN_ID) {
            ...
            switch (msg_id) {
            case STATUS_MSG_1: {
                uint32_t rpm = (uint32_t) ((data[0] << 24) | (data[1] << 16) | (data[2] << 8)
                                           | data[3]);
                printf("msg1, rpm %d", rpm);
                fb_tail_rpm::publish(rpm);
            ...
            case STATUS_MSG_2: {
                ...
                float apm_hours_charged = ((data[4] << 24) | (data[5] << 16) | (data[6] << 8)
                                           | data[7])
                                          / 10000.0;
            ...
            case STATUS_MSG_3: {
                ...
                float watt_hours_charged = ((data[4] << 24) | (data[5] << 16) | (data[6] << 8)
                                            | data[7])
                                           / 10000.0;
    ...
    EXPORT void on_serial(const uint8_t *data, size_t size)
    {
        ...
        case ECU_FSP_ID:
        case ECU_STATUS_ID:
            processEcuPackage(can_id, packData);
            break;
